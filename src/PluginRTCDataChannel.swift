...
class PluginRTCDataChannel : NSObject, RTCDataChannelDelegate {
	@objc var rtcDataChannel: RTCDataChannel?
	@objc var eventListener: ((_ data: NSDictionary) -> Void)?
	@objc var eventListenerForBinaryMessage: ((_ data: Data) -> Void)?
	@objc var lostStates = Array<String>()
	@objc var lostMessages = Array<RTCDataBuffer>()
	 ...
	 */
	@objc init(rtcDataChannel: RTCDataChannel) {
		NSLog("PluginRTCDataChannel#init()")
	...
	@objc func emitReceivedMessage(_ buffer: RTCDataBuffer) {
		if buffer.isBinary == false {
			let string = NSString(
				data: buffer.data,
